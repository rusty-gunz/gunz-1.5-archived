...
static INT_PTR CALLBACK EluDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	Interval animRange;
	MCplug2 *exp = (MCplug2*)GetWindowLong(hWnd,-GWLP_USERDATA); 
...
//	BOOL tm_ani = (options & SCENE_EXPORT_SCALE_ANI) ? TRUE : FALSE;
	...
	if(!suppressPrompts) {
		if (!DialogBoxParamA(hInstance, MAKEINTRESOURCE(IDD_ELU_EXPORT),ip->GetMAXHWnd(), EluDlgProc, (LPARAM)this)) 
			return 1;
	}
	else { // 맵에서 불리는 경우
