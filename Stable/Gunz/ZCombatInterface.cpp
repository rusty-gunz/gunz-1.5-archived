...
void ZCombatInterface::DrawBlitzScoreBoard(MDrawContext* pDC)
{
	...
	MFrame* bitMapFrame = dynamic_cast<MFrame*>(ZGetGameInterface()->GetIDLResource()->FindWidget("BlitzkriegScoreBoard"));
	if (bitMapFrame != nullptr)
	{
		bitMapFrame->Show(true);

		MPicture* scoreBoardBitmap = dynamic_cast<MPicture*>(ZGetGameInterface()->GetIDLResource()->FindWidget(bitMapName.c_str()));
		if (scoreBoardBitmap != nullptr)
		{
			scoreBoardBitmap->Show(true);
			//scoreBoardBitmap->Draw(pDC);
		}
	}
