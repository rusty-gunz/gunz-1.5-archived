...
//	SetListenerWidget("MapList_Caller", ZGetStageMapListCallerListener());
	...
	*/
	...
	SetListenerWidget("CashEquipmentListCaller", ZGetAllLadderEquipmentListCallerButtonListener());
	...
	SetListenerWidget("CashEquipmentList", ZGetShopPurchaseLadderItemListBoxListener());
	...
	SetListenerWidget("Shop_SubEquipmentFilter", ZGetShopListSubFilterListener());
	SetListenerWidget("Equip_SubEquipmentFilter", ZGetEquipListSubFilterListener());
	...
	////////////////////////////////////////////////////////
	SetListenerWidget("ShoptoEquipment", ZGetShopEquipmentCallerButtonListener());
	////////////////////////////////////////////////////////
...
//	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_CLASSIC_TEAM, "Team Deathmatch (Classic)");	
	...
	pCombo = (MComboBox*)m_IDLResource.FindWidget( "Shop_SubEquipmentFilter");
	if ( pCombo)
	{
		pCombo->SetAlignment( MAM_LEFT);
		pCombo->SetListboxAlignment( MAM_LEFT);
	...
	pCombo = (MComboBox*)m_IDLResource.FindWidget( "Equip_SubEquipmentFilter");
	if ( pCombo)
	{
		pCombo->SetAlignment( MAM_LEFT);
		pCombo->SetListboxAlignment( MAM_LEFT);
...
void ZGameInterface::ShowEquipmentDialog(bool bShow)
{
	...
	{	// ½ºÅ×ÀÌÁö ¾È¿¡¼­ Àåºñ ¹öÆ° ¼±ÅÃ
		...
		else if ( ZApplication::GetGameInterface()->GetState() == GUNZ_GAME)
		{
			ShowWidget( "Stage", false);
			ShowWidget( "Lobby", false);
			ShowWidget( "Shop", false);
		...
		if ( pButton && pLabel)
		{
			char buf[ 256];
				pButton->Show( true);
				sprintf( buf, "%s", ZMsg( MSG_WORD_EQUIPMENT));
				pLabel->SetText( buf);
		}
...
void ZGameInterface::ShowShopDialog(bool bShow)
{
	...
	{	// »óÁ¡ ¼±ÅÃ
		if ( ZApplication::GetGameInterface()->GetState() == GUNZ_STAGE)
		{
			ShowWidget( "Stage", true); //Enable Stage Widget
			ShowWidget( "Lobby", false);
			ShowWidget( "Equipment", false);
		}
		else if ( ZApplication::GetGameInterface()->GetState() == GUNZ_LOBBY)
		{
			ShowWidget( "Stage", false); //Disable Stage Widget
			ShowWidget( "Lobby", true);
			ShowWidget( "Equipment", false);
		}
		else if ( ZApplication::GetGameInterface()->GetState() == GUNZ_GAME)
		...
		MButton* pButton = (MButton*)pResource->FindWidget("ShoptoEquipment");
		if ( pButton)
			pButton->Show( true);
