...
{
	MPicture* parentWidget = (MPicture*)ZGetGameInterface()->GetIDLResource()->FindWidget("BlitzEvent");
	if (!parentWidget)
		return;
	parentWidget->Show(true);
...
void ZRuleBlitzKrieg::UpdateHPTopInfo()
{
	...
	if (pPicture)
	{
		...
		pPicture->SetBitmap(GetClassBitmap(ZGetGame()->m_pMyCharacter->GetBlitzClass(), ZGetGame()->m_pMyCharacter->GetTeamID()));
	...
	if (pPicture)
	{
		...
		{
			pPicture->Show(isUIVisible);
			pPicture->SetBitmap(GetClassBitmap(pObj->GetBlitzClass(), pObj->GetTeamID()));
		}
	}
	...
	if (pPicture)
	{
		pPicture->SetSize(pPicture->GetBitmap()->GetWidth(), 16);
		pPicture->SetPosition(140.f / 800.f, 100.f / 600.f);
		pPicture->Show(isUIVisible);
	}
	...
	if (pPicture)
	{
		pPicture->SetSize(pPicture->GetBitmap()->GetWidth(), 12);
		pPicture->SetPosition(140.f / 800.f, 100.f / 600.f);
		pPicture->Show(isUIVisible);
	}
	...
	{
		pPicture->SetSize(pPicture->GetBitmap()->GetWidth(), 16);
		pPicture->SetPosition(84.f / 800.f, 80.f / 600.f);
		pPicture->Show(isUIVisible);
	}
	...
	//Labels here
	{
		...
		if (pLabel)
		{
			pLabel->SetText(ZGetGame()->m_pMyCharacter->GetTeamID() == MMT_RED ? to_string(m_redBarricades).c_str() :
				to_string(m_blueBarricades).c_str());
		}
		pLabel = (MLabel*)ZGetGameInterface()->GetIDLResource()->FindWidget("BlitzBarricadeCount_Second");
		if (pLabel)
		{
			pLabel->SetText(ZGetGame()->m_pMyCharacter->GetTeamID() == MMT_RED ? to_string(m_blueBarricades).c_str() :
				to_string(m_redBarricades).c_str());
		}
	...
	//ZBmNumLabels
	{
		...
		{
			char szText[24];
			sprintf_s(szText, "%2d,%02d", GetElapsedTime() / 1000 / 60, (GetElapsedTime() / 1000) % 60);
